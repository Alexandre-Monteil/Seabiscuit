...
"""
                ...
                prompt += "\nDeliver a high-impact 4-part dossier: 1. Executive Summary & Alpha Verdict, 2. Equine Stock Valuations (+EV Long, Short Fade, Strangle), 3. Track Moisture & Speed Velocity Delta, 4. Recommended Wall Street Bet Portfolio & Risk Hedging Strangle."
        ...
        hedges = [a for a in equity_assets if a.get("asset_tag") == "MID_TIER_HEDGE"]
...
## 💡 RECOMMENDED WALL STREET BET PORTFOLIO & RISK STRANGLE

> [!IMPORTANT]
> **Primary Long Position (60% Capital)**: Buy `{top_buy.get('ticker')}` at ${top_buy.get('share_price_usd'):.2f}/share or win odds >= {top_buy.get('decimal_odds')}.
>
> [!WARNING]
> **Primary Short/Fade Position (25% Capital)**: Short/Lay `{top_fade.get('ticker')}` at ${top_fade.get('share_price_usd'):.2f}/share (Odds <= {top_fade.get('decimal_odds')}).
>
> [!NOTE]
> **Hedge Strangle (15% Capital)**: Place Dutch cover on Mid-Tier asset `{hedges[0].get('ticker') if hedges else '$GOLD'}` for downside volatility protection.
"""
